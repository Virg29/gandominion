...
export class ClarificatePlayMenu {
  ...
  clarify(
    ...
    this.callback = cb;
    Table.showMessage(
      `Played By ${data.PlayedBy != null ? CardEnum[data.PlayedBy] : 'nothing'}, Card is ${
        CardEnum[data.PlayedCard]
      }`,
    );
  ...
  addToSelected(name: string) {
    ...
    this.selectedCards.push(
      new ClarificateSelectedCard(this.drawOn, name, getCardImageUrlByName(name), position),
    );
  }
  ...
  renderCards(cardIds: number[]) {
    cardIds.forEach((id, index) => {
      const position = {
        x: (PLAY_AREA_SIZE.x / cardIds.length) * index + PLAY_AREA_POS.x + 10,
  ...
  private initializeButton() {
    this.createImage(ClarifyPlayButtonImg, { x: 1600, y: 550 }, 0.2, (img) => {
      this.button = img;
  ...
  private handleSelection() {
    ...
    const args = names.map((name) => Object.keys(CardEnum).indexOf(name)) as unknown as number[];
...
export class ClarificateCard extends ClarificateCardBase {
  constructor(drawOn: Group, name: string, url: string, position: Vector2d) {
    super(name, drawOn, url, position, 0.5, () => ClarificatePlayMenu.instance.addToSelected(name));
  }
}
